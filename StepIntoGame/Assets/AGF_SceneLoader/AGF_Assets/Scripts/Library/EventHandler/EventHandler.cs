...
/* 
 *  Setup:
 *  Create an empty GameObject and add the EventHandler script to it.
 *  Create custom event classes that extend the CustomEvent.
 * 
 *  Restrictions & Tips:
 *  DO NOT add event listeners in the Awake() method! 
 *  This is used by the EventHandler to initialize.
 ...
 * 	void Start() {
 * 		EventHandler.instance.addEventListener(CustomEventObj.EVENT_TO_LISTEN_TO, gameObject, "OnSomethingHappened");
 *  }
 *  
 ...
 * 			EventHandler.instance.removeEventListener(CustomEventObj.EVENT_TO_LISTEN_TO, gameObject);
 * 			// remove all events
 * 			EventHandler.instance.removeAllEventListeners(gameObject);
 * 		}
 * 	}
 * 
 ...
 * 		EventHandler.instance.dispatchEvent(evt);
 * 	}
 * 
...
public class EventHandler : MonoBehaviour {
	...
	// settings
	public bool allowSingleton = true; // EventHandler class will transfer between scene changes.
	private bool allowWarningOutputs = false;
	private bool allowDebugOutputs = false;
	...
	private void Awake() {
		if (!_created && allowSingleton) {
			...
			if (allowSingleton) {
				if (EventHandler.instance.allowWarningOutputs) {
